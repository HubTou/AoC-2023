...
def push_button(modules, todo):
    ...
    while todo.qsize():
        ...
        if target == "rx" and pulse == "low":
            return modules, todo, high_pulses, low_pulses, True
    ...
    return modules, todo, high_pulses, low_pulses, False
...
# 2) Loop until rx receives a low pulse
machine_started = False
todo = queue.SimpleQueue()
modules, todo, high_pulses, low_pulses, machine_started = push_button(modules, todo)
button_presses = 1
while not machine_started:
    modules, todo, hp, lp, machine_started = push_button(modules, todo)
    high_pulses += hp
    low_pulses += lp
    button_presses += 1

print(f"{button_presses} button presses")
